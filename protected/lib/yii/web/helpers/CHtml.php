...
if($live)
   $cs->registerScript('Yii.MyCHtml.#' . $id, "$('body').off('$event','#$id'); 
      $('body').on('$event','#$id',function(){{$handler}});");
else
   $cs->registerScript('Yii.MyCHtml.#' . $id, "$('#$id').off('$event'); 
      $('#$id').on('$event', function(){{$handler}});");                        
